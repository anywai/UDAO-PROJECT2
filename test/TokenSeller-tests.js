...
describe("TokenSeller", function () {
    ...
    it("Should revert when non-owner set UDAO contract", async function () {
        await expect(tokenSellerContract.connect(user1).setUDAO(udaoContract.address)).to.be.revertedWith("AccessControl: account " + user1.address.toLowerCase() + " is missing role " + await tokenSellerContract.DEFAULT_ADMIN_ROLE());
    }
    );
