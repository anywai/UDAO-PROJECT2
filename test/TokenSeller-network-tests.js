...
describe("TokenSeller", function () {
    ...
    this.afterEach(async function () {
        console.log("TokenSeller contract was deployed to:", tokenSellerContract.address);
        console.log("UDAO contract was deployed to:", udaoContract.address);
    });
    ...
    it("Should allow owner to set UDAO contract", async function () {
        tx = await tokenSellerContract.setUDAO(udaoContract.address);
        await tx.wait();
    ...
    it("Should emit UDAOSet event when owner set UDAO contract", async function () {
        await expect(tokenSellerContract.setUDAO(udaoContract.address)).to.emit(tokenSellerContract, "UDAOSet").withArgs(udaoContract.address);
    }
    );
    it("Should revert when non-owner set UDAO contract", async function () {
        tx = await tokenSellerContract.connect(user1).setUDAO(udaoContract.address);
        await tx.wait();
        // Get the revert reason from the tx receipt
        const txReceipt = await ethers.provider.getTransactionReceipt(tx.hash);
        const revertReason = helpers.decodeRevert(txReceipt);
        expect(revertReason).to.equal("AccessControl: account " + user1.address.toLowerCase() + " is missing role " + await tokenSellerContract.DEFAULT_ADMIN_ROLE());
    }
    );
    ...
    it("Should revert when non-owner release UDAO tokens", async function () {
        await tokenSellerContract.setUDAO(udaoContract.address);
        await udaoContract.transfer(tokenSellerContract.address, 1000);
        await expect(tokenSellerContract.connect(user1).releaseTokens()).to.be.revertedWith("AccessControl: account " + user1.address.toLowerCase() + " is missing role " + await tokenSellerContract.DEFAULT_ADMIN_ROLE());
    }
    );
    ...
    it("Should emit BalanceAdded event when owner add token balance for a single user", async function () {
        ...
        await udaoContract.transfer(tokenSellerContract.address, 1000);
        await tokenSellerContract.releaseTokens();
        await expect(tokenSellerContract.addBalance(user1.address, 100)).to.emit(tokenSellerContract, "BalanceAdded").withArgs(user1.address, 100);
    }
    );
    ...
    it("Should revert when size of addresses and amounts are not equal for batchAddBalance", async function () {
        await expect(tokenSellerContract.batchAddBalance([user1.address, user2.address, user3.address], [100, 200])).to.be.revertedWith("Arrays must be the same length!");
    }
    );
    ...
    it("Should emit TokensWithdrawn event when a user withdraw tokens", async function () {
        ...
        await tokenSellerContract.releaseTokens();
        await tokenSellerContract.addBalance(user1.address, 100);
        await expect(tokenSellerContract.connect(user1).withdraw()).to.emit(tokenSellerContract, "TokensWithdrawn").withArgs(user1.address, 100);
    }
    );
