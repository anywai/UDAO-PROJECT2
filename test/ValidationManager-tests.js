...
describe("UDAO Project", function () {
    ...
    it("Should not validate content if not exist", async function () {
        ...
        await expect(contractValidationManager.connect(contentCreator).setAsValidated(voucher)).to.revertedWith('ERC721: invalid token ID')
    });
    ...
    it("Should not validate content if redeemer is not owner of token", async function () {
      ...
      await expect(contractValidationManager.connect(contentBuyer).setAsValidated(voucher)).to.revertedWith("Redeemer is not the owner of the token");
  });
});
