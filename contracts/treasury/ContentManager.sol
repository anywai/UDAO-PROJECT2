...
abstract contract ContentManager is BasePlatform {
    ...
    mapping(address => mapping(uint256 => uint256[])) ownedParts; // ilk değeri 0 olmalı! 0. indisi kullanılamaz
    ...
    function _buyContentwithUDAO(
        ...
        for (uint256 j; j < purchasedParts.length; j++) {
            ownedPartIndex[contentReceiver][tokenId][purchasedParts[j]] = (
                ownedParts[contentReceiver][tokenId].length
            );
            ownedParts[contentReceiver][tokenId].push(purchasedParts[j]);
        }
