...
async function main() {
  ...
  await expect(contractRoleManager.grantRole(BACKEND_ROLE, backend.address)).to.emit(contractRoleManager, "RoleGranted");
  console.log("Backend role granted to backend")
  //tx = await contractRoleManager.grantRole(CONTENT_PUBLISHER, backend.address);
  await expect(contractRoleManager.grantRole(CONTENT_PUBLISHER, backend.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(CONTRACT_MANAGER, contractContractManager.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(VOUCHER_VERIFIER, backend.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(SALE_CONTROLLER, backend.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(FOUNDATION_ROLE, foundation.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(TREASURY_CONTRACT, contractPlatformTreasury.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(DEFAULT_ADMIN_ROLE, foundation.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(SUPERVISION_CONTRACT, contractSupervision.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  await expect(contractRoleManager.grantRole(UDAOC_CONTRACT, contractUDAOContent.address)).to.emit(contractRoleManager, "RoleGranted");
  ...
  console.log("Version 1 contracts addresses synced in contractContractManager")
