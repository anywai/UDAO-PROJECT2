...
async function main() {
  ...
  contractUDAO = ethers.utils.getAddress("0x433cCeBC95ad458E74d81837Db0D4aa27e30E117");
  ...
  console.log("Every deployment will wait for " + WAIT_BLOCK_CONFIRMATIONS + " confirmations.");
  console.log(
  ...
  await tx.wait();
  console.log("DEFAULT_ADMIN_ROLE of contractRoleManager granted to foundation address");
  ...
  tx = await contractPlatformTreasury.connect(backend).setFoundationAddress(foundation);
  await tx.wait();
  ...
  tx = await contractUDAOCertificate.connect(backend).setFoundationAddress(foundation);
  await tx.wait();
  console.log("Foundation address set as ", foundation);
  // Set foundation address in voucherverifier contract
  console.log("Setting foundation address in VoucherVerifier...");
  tx = await contractVoucherVerifier.connect(backend).setFoundationAddress(foundation);
  await tx.wait();
  console.log("Foundation address set as ", foundation);
...
*/
  ...
  console.log("Manually transfer the admin role of contractRoleManager to foundation address");
}
