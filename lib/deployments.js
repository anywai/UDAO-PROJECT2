...
async function deploy(isGovernanceContractsReplaced = false) {
  ...
  if (isGovernanceContractsReplaced) {
    ...
    console.log("Error?2")
    await contractSupervision.connect(backend).updateAddresses(contractRoleManager.address, contractUDAOContent.address, contractGovernanceTreasury.address, contractUDAOStaker.address);
