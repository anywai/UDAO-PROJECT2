...
async function main(isDexRequired = false) {
  ...
  if (process.env.network == "localhost" && process.env.network  == "hardhat") {
    ...
    await helpers.setBalance(backend.address, ethers.utils.parseEther("100000"));
    console.log("backend amount of matic: ", await backend.getBalance())
    // Send 1000 ETH to foundation wallet
    await helpers.setBalance(foundation.address, ethers.utils.parseEther("100000"));
...
/// TODO activate below. For some reason setting wallet balance won't work in if/else statement
/*
 // Send 1000 ETH to backend wallet
 await helpers.setBalance(backend.address, ethers.utils.parseEther("100000"));
 console.log("backend amount of matic: ", await backend.getBalance())
...
*/
  ...
  console.log("Setting contractPlatformTreasury.address as platform treasury address in contractContractManager: ", contractPlatformTreasury.address)
  await contractContractManager.connect(backend).setPlatformTreasuryAddress(contractPlatformTreasury.address);
  console.log("Setting contractUDAOVp.address as udao-vp address in contractContractManager: ", contractUDAOVp.address)
